...
默认情况下，`git pull` 会将远程分支的 commit 以 `merge` 的方式追加到本地分支之上，这经常会导致作混乱的 commit history。更多的情况下，作为下游的本地分支 `rebase` 到作为上游的远程分支是更好的做法。

为此，`git pull` 命令提供两种策略：`--merge` 和 `--rebase`，其中前者为默认策略。要想默认使用 `--rebase`，可通过以下命令设置：
